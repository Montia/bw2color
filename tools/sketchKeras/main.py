...
def get(path, to, mod):
    # print(path)
    from_mat = cv2.cvtColor(np.asarray(path), cv2.COLOR_RGB2BGR)
    ...
    if mod is None:
        line_mat = light_map.transpose((3, 1, 2, 0))[0]
    else:
    ...
    i = show_active_img_and_save_denoise('sketchKeras', line_mat, to)
    cv2.waitKey(0)
    return i
